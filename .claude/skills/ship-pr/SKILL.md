...
1. **Rebase on latest main.** The remote is `upstream` in the maintainer's checkout
   (`origin` in a Claude cloud clone — substitute it below):
...
```sh
git push -u upstream HEAD          # push the branch (`origin` in a cloud clone)
