...
- **Tables scroll, never squeeze.** A markdown table renders inside a scroll host (`chat/tables.ts`
  ...
  it, arrow keys scroll — for as long as it overflows; one that fits is none (`shared/scrollRegion.ts`,
  re-checked when the transcript or the table's own width changes; a hidden pane catches up when
  shown).
