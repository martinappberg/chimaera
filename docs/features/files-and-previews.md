...
- **What the agent gets.** One line, `@<path>#<locator> (<context>) "<quote>"`, each part after the
  ...
  | table cells / rows | `@de.tsv#cell=5,2-9,4 "log2FC\tpadj\n2.05\t0.005…"`, `@de.tsv#row=12-14 "…"` |
...
- **Key behaviors.**
  ...
  - **Table rows count data rows** (the grid's own numbers; RFC 7111 syntax, the header line not
    counted). Spreadsheet A1 follows the sheet: a table whose used range starts at C4 references
    D6, not B2.
...
- **Pending.** The one-selection model (no basket of several spots), crops uploaded rather than
  pasted into terminal agents, table rows counted from the grid's numbers rather than RFC 7111's
  header-inclusive count, and the chip-only affordance (no context menu) have not been confirmed
