...
see Icons below) · `export --format pptx|pdf|svg|svg-outlined` · `theme-export --format
...
Board is a **composition tool, not just a diagram builder**: a bundled icon
set lets the agent place real artwork by name, so figures are richer than
boxes and arrows. The set is the **Tabler outline** collection (v3.45.0, MIT —
5,112 glyphs), committed under `crates/chimaera-board/src/icons/` as one
compact manifest (~900 KB of path data + a discovery index + the license,
regenerated by `generate.py`; the daemon never reads node_modules at build
time). It is default-on behind the `icons` cargo feature — skippable like
`math` for a smaller binary, in which case the object renders a
placeholder-with-reason and `chimaera board icons` says so.

- **Discovery** — `chimaera board icons <query>` fuzzy-matches names *and*
  Tabler's synonyms/categories (`flask`, `arrow`, `dna`, `laboratory` →
  `flask`), prints capped matches with a placement snippet and the total, so
  one call takes a concept to a placeable name. `--list` prints the total.
- **The `icon` object** — `{"type":"icon","name":"flask","at":[x,y],
  "size":[w,h],"color":"@accent1","strokeWidth":1.5}`. The 24×24 paths fit the
  box aspect-preserving and centered; `color` is any `@token` or `#hex`
  (default `@fg`), so recoloring is a token swap; `strokeWidth` is Tabler's
  own scale (default 2) and scales with the placed size. `name` is required —
  an unknown name renders a visible placeholder and is a **lint error**.
- **Node icons** — a `diagram` node takes an optional `icon` drawn leading its
  label, sized to the node height, with the layout widened to fit; the direct
  fix for a "too boring" flow. Mermaid import leaves nodes iconless (its
  flowchart subset has no clean icon slot).
- **PPTX** — an icon exports as a **group of editable `custGeom` vector
  shapes** (one per outline path, arcs flattened to cubics per rule 1, stroke
  = the resolved token), so the user can drag it down and polish in
  PowerPoint. Icons compose with imported SVG/PNG and native shapes into a
  single editable figure.
