...
## PR E — daemon shared-work extras (2026-08-31)

The audit's "bounded but duplicated" daemon costs (perf-plan item 7 / the "daemon extras" list), plus one harness bug. None were idle-hot; all scaled per connected window. Zero wire-shape changes — every frame's bytes are exactly what each client built for itself before.

- **Shared `/ws/events` sessions snapshot** (`session_view::SnapshotCache` + `state::ChangeBus`). Each connected events client woke on the same change notify (up to 4 Hz during chat turns — every coalesced chat event fires `notify_waiters`) and rebuilt + serialized the full sessions snapshot itself: N windows, N identical builds. Now `state.changes` is a `ChangeBus` — the same `Notify`, plus a generation counter bumped on every wake (same method names, so all ~40 call sites compiled unchanged) — and the serialized frame is cached once per generation and fanned out as an `Arc<String>`. The cache lock is held *across* the build deliberately: that is the single-flight (the build is fully synchronous; nothing inside it takes the cache lock). Two subtleties: the generation is read BEFORE the build, so a change landing mid-build stamps a newer generation and forces the next reader to rebuild (a spare build, never staleness); and the cache also expires on a 250 ms clock within a generation, because `stalled`/`output_active` are time-derived and must keep flipping on the 1s fallback tick with no generation bump. Per-client state (fs_watch, git-epoch/settings/update/recents sends, the last-sent compare) stays per-client; the compare got an `Arc::ptr_eq` fast path.
- **Single-flight `git status`** (`git/service.rs::StatusShare`). An epoch bump makes every watching window refetch `GET /git/status` at once — up to 4 concurrent identical porcelain-v2 runs on a big repo. Now per workspace: one async-mutex slot joins concurrent callers (the leader runs, followers wait and reuse) with a ~1s reuse window stamped at run *start*. The part that took thought: event-driven invalidations (`GitService::invalidate`, fired by `mark_path_dirty` after a save/agent write) bump an atomic flush counter that both expires the cached result *and* stops a mid-flight run from installing data that predates the announced change — so a post-save refetch can never be served a pre-save status (the earlier draft would have wedged stale until the 12s backstop). Errors are never cached. The HTTP handler, the backstop poll, and the MCP `git_facts` all ride the share; the existing semaphore, 8s timeout, and output caps are untouched underneath.
- **Settings stat off the reactor** (`settings.rs::watch_external_edits`). `send_settings_snapshot` re-statted settings.json on the reactor under the settings mutex on every events wake — an NFS hiccup there stalls the whole reactor. The events path now reads only the cached generation/map (`generation_cached`/`map_cached`, no stat); hand-edit detection moved to one daemon-wide 2s poll that stats off-reactor (`spawn_blocking`), reloads only when the mtime actually moved, and fires `notify_waiters` so every window converges. REST reads (`current()`) still refresh inline, so `GET /settings` freshness is unchanged; the broadcast latency for a hand-edit goes from "next events wake" to ≤~2s + the wake — invisible for an action that was already asynchronous to every window.
