...
Follow-up to the live performance audit: one busy *hidden* terminal measured 11-18% daemon CPU + 10-17% renderer CPU, continuously, because the client pool parked detached xterms with sockets open and parsed every byte anyway — × up to POOL_CAP=12.

- **Parked terminals buffer, don't parse** (`termPoolRuntime.ts`). While an entry sits in the hidden stash, `onBinary` appends raw bytes to a bounded per-entry buffer (512 KiB) instead of `term.write`; adopt flushes the buffer in order before live writes resume. Overflow discards the buffer (a truncated escape stream would corrupt the grid) and marks needs-resync: adopt forces a clean socket reconnect, and the server's fresh attach re-snapshots the authoritative grid. A reset that arrives while parked (server resync/reconnect) drops the stale buffer and is replayed resize-before-reset ahead of the flush — the grid-adoption ordering. Verified nothing depends on parked parsing: titles, cwd, and busy/idle are daemon-derived (`EventProxy` OSC titles, the cwd poller, `last_output_at`→`output_active`); the link prefetch hooks `term.onRender`, inert while hidden. The one deferred side effect is client-side OSC 52 clipboard writes, which now land on adopt (or never, after overflow) — a hidden session silently setting the clipboard was the worse behavior.
- **PTY output coalescing** (`ws.rs`). The bridge previously sent one binary WS frame per PTY read (≤8 KiB) — dozens of frames/sec per client under a repainting TUI. Now: leading edge (the first chunk after an idle gap sends immediately, so typing echo never waits), then an 8 ms window / 32 KiB ceiling per frame, with an opportunistic `try_recv` drain so already-queued chunks batch for free. Single-chunk frames still ride the refcounted broadcast `Bytes` (zero-copy per client). Lagged drops the pending batch and resyncs, as before; a Closed with a window open flushes the batched tail first — those are the session's last words. Frame *boundaries* changed; frame semantics and the auth/ready/snapshot handshake did not.
- **Snapshot renders off the reactor** (`ws.rs`). Attach and resync renders (full scrollback under the term std::Mutex — measured ~95 ms at the 10k default, cap 200k) now run under `spawn_blocking`, so one attach can't stall a reactor worker. The render still briefly stalls that session's PTY reader thread (term lock) — accepted; rendering from a grid copy was considered and deferred. New invariant recorded in `.claude/rules/pty.md`.
- **WebGL retry on adopt** (`termPoolRuntime.ts`). A context loss used to downgrade a pooled terminal to the DOM renderer permanently; adopt now retries the WebGL addon once per adoption (construction failure stays permanent — that means WebGL is genuinely unavailable).
