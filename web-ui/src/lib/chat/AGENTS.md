...
| `tables.ts` | Marked renderer override hosting every GFM table in a `.md-table` scroll container — emitted in the HTML string BEFORE sanitize, because the `{@html}` hosts are append-only after render (no post-render DOM wrap); reuses marked's default table markup via `Renderer.prototype`, so cell rendering and streaming ⇄ settled parity are untouched. The CSS side (host scrolls, cells keep whole tokens, headers stay one line) lives in `Markdown.svelte`. Own vitest pin (`tables.test.ts`). |
