...
| `ToolCallCard` / `ToolGroup` | Tool-call rendering (title, status, diff/output, grouping). Terminal rows may accept late output text but must never revive their streaming cursor. ToolGroup also renders the run's `board show` cards first-class AFTER the collapsed group (the summary counts them; the producing row keeps a compact reference). |
...
| `ShownCard.svelte` / `shownBoards.ts` | The `chimaera board show` figure card, first-class in the conversation flow: `shownBoards.ts` (vitest-covered) detects the `shown … → *.board` line (a legacy `.board.json` path still matches) in completed results (client-side v1 — a daemon `shown` journal event can replace the detection) and reduces transcript-wide so a same-`--id` re-show updates ONE card in place at its latest position; the card renders the server-side content-addressed PNG, opens the board pane on click, and carries hover actions — a provenance "data" disclosure (origin/source/inputs/trace parsed from the board file) and "save to boards/" (explicit promotion via `/fs/mkdir` + `/fs/copy`, compound-extension-safe unique naming). |
