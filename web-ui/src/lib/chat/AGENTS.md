...
| `ActivityFold.svelte` / `activityFold.ts` / `ActivitySummary.svelte` | Settled activity folds: `foldSpans` (pure, own vitest suite) marks each run of ≥2 activity items (thought, tool group, finished) that an agent message directly follows; ChatView renders it as one `ActivityFold` line titled by `foldTitle` ("Thought, ran 6 commands, read 2 files"), whose rows mount only while open and render through the same `activityRow` snippet as the live column. `ActivitySummary` is the one-line disclosure (label, live dot, failed/recovered badge, chevron) the fold and `ToolGroup` share; `toolRunHealth` (`toolLabels.ts`) computes the badge. |
...
- **Prose leads; activity lines follow.** Thought, tool-group, finished and wake rows share one
  ...
  weight. A run of activity that a reply has followed folds into one line (`activityFold.ts`); the
  trailing run stays unfolded, so live work is always visible. Wake markers are reducer-derived (`markWake`, pure over `blocks` + the live background
