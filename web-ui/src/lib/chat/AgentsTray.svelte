<script lang="ts">
   ...
   * instead of scrolling off in the chat. Each agent shows its latest progress
   * line (tools · tokens, from task_progress) and a stop button; when an agent
...
<div class="tray" role="status" aria-live="polite">
  <div class="tray-head">
    ...
    <span>{agents.length === 1 ? "subagent working" : `${agents.length} subagents working`}</span>
  </div>
