<script lang="ts">
  ...
  /** The pending SSH auth prompt (password / 2FA), or null. */
  let askpass = $state<AskpassPrompt | null>(null);
  /** What the user has typed into the prompt's field. */
...
{#if askpass !== null}
  <div
    ...
    role="presentation"
    onclick={cancelAskpass}
    onkeydown={(e) => e.key === "Escape" && cancelAskpass()}
  >
    ...
    >
      ...
      <div class="askpass-actions">
        <button class="askpass-cancel" onclick={cancelAskpass}>cancel</button>
        <button class="askpass-go" onclick={submitAskpass}>authenticate</button>
      </div>
    </div>
  </div>
{/if}
