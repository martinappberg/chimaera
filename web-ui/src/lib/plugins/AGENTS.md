...
| File | What it owns |
|---|---|
| `store.ts` | Wire types + fetchers for every plugin route (workspace plugins, `PUT …/{pid} {on}`, agent-plugins, skills, install / setup / trust-hooks; the installed-plugin routes `POST /plugins/install`, `/plugins/{pid}/update` · `rollback` · `check`, `DELETE /plugins/{pid}`), the active workspace's reactive plugin status (`workspacePlugins`, `knowledgeProviderActive`, `myceliumPlugin`; `changeWorkbenchPlugin` runs one change and re-syncs the cards and Knowledge), the daemon's own version from `/health` (`daemonVersion`, for "ships with chimaera <version>"), and the attach-sheet request (`attachRequest` / `openAttachSheet` / `closeAttachSheet`) that App.svelte hosts as ONE modal for every surface. `WorkspacePlugin` carries the version wire: `version`, `api`, `source` (`embedded` / `installed`), `stale`, and — null when absent — `path`, `embedded_version`, `installed_version`, `previous`, `update`, `fault`. |
...
| `InstalledView.svelte` | Workbench cards (glyph tile · name · the plugin's own `version` · a source chip — "ships with chimaera <daemon version>", "installed", or "installed · x ships with chimaera" when both copies exist — a `stale` chip, and an **Update to x.y.z** chip when a check found one · summary · `Switch` = on in THIS workspace · Here / Adds ("Would add" when off) / Needs lines; per-agent requirement chips with Install and "Review & trust →"; for an installed copy an Installed line with **Use previous (x)**, **Check now** and **Remove** (a `ConfirmDialog` naming the versions it deletes); the `fault` when it can't run; one outcome line per change — an update shows the verified `plugin.wasm` sha256), then the Agents section (what each CLI reports about its own plugins; the agent plugin's version rides its Needs pill). |
...
- **Versions and sources come from the daemon** (the catalog decides
  precedence and the gates); the card never compares versions itself. The
  daemon never downloads on its own: Update, Use previous and Remove are the
  user's clicks, each re-syncing the cards.
