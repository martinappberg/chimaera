...
| `journal.rs` | Per-session append-only JSONL + bounded replay ring + the native-id→session index (+ per-thread effort) + the per-agent-kind `AgentPrefsStore` (last model/effort, `prefs.json`) + dir pruning. The gap-replay crown jewel. | anything touching durability, replay, seq numbering, or what a new chat starts with. |
