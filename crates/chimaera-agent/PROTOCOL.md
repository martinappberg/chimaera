...
thread open from its persisted per-conversation/latest choice or Codex's
`config/read` value, seeds the effort chip from the open response, applies
selections through `thread/settings/update`, and journals the read-back. The
explicit `turn/start.effort` remains the compatibility path when
settings/update is absent. Plan mode must update both the top-level `effort`
and its nested `collaborationMode.settings.reasoning_effort` or the stale
nested value can win.
...
`turn_context.effort:"xhigh"`, but `thread/resume` returned `low` and the next
turn's native context was genuinely `low`. This was not a guardian leak or a
stale header: app-server persisted the conversation without rehydrating its
selected effort.

The generated 0.144.x schema also defines
`config/read {cwd?,includeLayers?}` and exposes
`config.model_reasoning_effort`. The handshake now makes that optional,
two-second-bounded read before opening a thread. Chimaera's own durable,
200-entry native-thread index carries the authoritative parent effort across
...
## Pass 31 (2026-09-23 — live probes claude 2.1.280/2.1.281 + codex 0.156.1): narration, tool labels, subagent/background/monitor lifecycles, reasoning summaries. ADOPTED.

The trigger was a live Opus 5.5 session whose interim prose — the sentences between tool calls that the Claude apps show as ordinary text — rendered inside chimaera's collapsed thinking blocks. Sources: the live journal and on-disk transcript of that session (claude 2.1.280 on an HPC login node), three paid probes (Opus 5.5 narration turn $0.15, a Sonnet foreground-subagent turn $0.38, a Sonnet background-Bash + Monitor turn $0.30, one Haiku `--replay-user-messages` Monitor probe), the 2.1.259 vs 2.1.281 binaries' SDK schemas (the zod `.describe()` text is the SDK contract), and the Claude desktop bundle. Pins bumped 2.1.259 → 2.1.281 and 0.153.0 → 0.156.1.

### Claude: narration is prose shipped as a thinking block

Opus 5.5 (at least) no longer always emits the prose between tool calls as `text`: the server summarizes it and ships it as a `thinking` block. The block kind is a tag inside the `signature` envelope — base64 → protobuf field 2 → field 1 → string field 8: `"narration"` vs `"thinking"` (live signatures decode to `…B\tnarration…` / `…B\x08thinking…`). The CLI's own classifier (`dne`/`$Rt` in 2.1.281) walks exactly that path, fail-closed. 2.1.280+ also names them on the assistant frame's wrapper: `narration_block_indexes: [i…]` ("thinking blocks whose signature the server tagged block_kind 'narration' (server summaries of the prose between tool calls, not the model's own reasoning)… A listed block can have empty thinking text… give those ordinary thinking treatment"), plus a VS-Code-only `narration_hint: hidden|faint`. 2.1.259 has the classifier but not the wrapper field. The TUI renders a narration block as prose (with a faint "· summarized" hint on some models) and ordinary thinking only in transcript mode; the Claude apps render narration as plain assistant text. Interim prose still sometimes arrives as a normal `text` block — both occur in one turn.

Live stream order (Opus 5.5, `--include-partial-messages`): `content_block_start {thinking, signature:""}` → every `thinking_delta` in one ~10 ms burst → `signature_delta` (the tag) → the per-block `assistant` frame (with `narration_block_indexes: [0]`) → `content_block_stop`. Ordinary summarized thinking streams its deltas over seconds before its signature.

ADOPTED: the driver holds a streaming thinking block's text OUTSIDE the coalescer (the harness flushes that every tick) until the block is classified — by `signature_delta` (stream time), else by the per-block assistant frame (`narration_block_indexes`, else the decoded signature), else at `content_block_stop` (thought). A block still streaming after `THINKING_HOLD_TICKS` (~300 ms after its first text) or past 8 KiB is reasoning and flows as thought from then on, so real thinking is delayed by at most the hold. Narration joins the MESSAGE stream (its own paragraph break), reasoning the thought stream. A narration block a hold timeout already released as thought is also emitted as prose (the thought copy stays; the prose is what matters). Turn ends, the interrupt watchdog and teardown release a held block as thought. The non-streamed assistant path and the transcript importer (`transcript.rs`, which reads on-disk transcripts that carry signatures but never the wrapper field) classify the same way.

### Claude: per-batch tool labels (`tool_use_summary`)

`{type:"tool_use_summary", summary, preceding_tool_use_ids[], uuid, session_id, timestamp?}` — a small-model ("Label:") past-tense summary of one tool batch ("Listed files in directory", "Printed a.txt and b.txt", "Counted directory files"). Emitted only when the host sets `CLAUDE_CODE_EMIT_TOOL_USE_SUMMARIES` (the CLI's `gates.emitToolUseSummaries`; the Claude desktop app sets it behind a feature gate), only for the main thread (`!agentId`), and it lands about one model round late (after the next API call, or just before the `result`). Each label costs one small-model call on the user's account. ADOPTED: `AgentEvent::ToolSummary {summary, tool_ids}` (capped); the daemon sets the env for Claude chat spawns unless `chat.toolSummaries` is off (default on). The UI titles a tool group with its labels and keeps counting the still-unlabelled tail ("Listed files · running a command").

### Claude: subagent lifecycle (2.1.281)

`task_started` gained `is_backgrounded`, `spawn_depth`, `skip_transcript`, `ambient`; `task_updated.patch` gained `is_backgrounded` (a foreground agent moved to the background), `total_paused_ms`, `error`, and statuses `killed | paused`; `task_notification` gained `reason: "worker_restart"` and `resource_links`. A FOREGROUND subagent also closes with `task_notification {task_id, tool_use_id, status, output_file, summary, usage:{total_tokens, tool_uses, duration_ms}}` — and `summary` is the agent's clean final report (live) — right before its Agent `tool_result`. That tool_result now wraps the report in model-facing scaffolding: `"[Subagent hand-back] … The report follows:\n"` + every report line indented two spaces + `agentId: … (use SendMessage …)` + `<usage>…</usage>`; the frame's `tool_use_result.content` holds the clean report. Every running agent also drives `system/task_summary {detail}` — the CLI's spinner phrase ("Counting files"; `detail: null` clears it, after the result too).

ADOPTED: `AgentEvent::SubagentFinished {id?, label, status, result?, stats?}` once per subagent from `task_notification` (this turn's agents via `task_rows`; a backgrounded agent that outlived its turn via its `local_agent` lane) — a subagent's close never also yields a generic background-close notice. `stats` is "2 tools · 12.3k tokens · 4s". Agent rows show the report with the hand-back frame stripped (live and import). Progress lines prefer the frame's `description` ("Running wc -c") over the bare tool name. `task_summary` → `AgentEvent::ActivityLine {detail?}` (latest-wins); each API call's `message_delta.usage.output_tokens` (per call; the result's total is their sum) plus the running thinking estimate → `AgentEvent::TurnTokens {output}` — together the status line's "9s · 1.4k tokens · 1 running task · Measuring file sizes…".

### Claude: background commands and Monitor watches

A `Monitor` tool call rides the same `local_bash` lane as a backgrounded Bash (`task_started {task_type:"local_bash", tool_use_id, is_backgrounded:true}`); its tool_result is "Monitor started (task …, expires in 30s …)" and its close `task_notification {summary:'Monitor "<desc>" stream ended'}`. Each monitor EVENT is injected into the CLI's own queue as a user message `{origin:{kind:"task-notification"}, content:"<task-notification><task-id>…</task-id><summary>Monitor event: \"<desc>\"</summary><event>tick 1</event>…"}` and wakes the agent into a turn — but that message never reaches stdout, not even with `--replay-user-messages` (probed); a host sees only the unprompted reply. `background_tasks_changed` entries and task frames carry `ambient` ("Ambient/housekeeping task. Consumers should hide this from the inline transcript; it may still appear in a tasks panel"). A backgrounded command's close summary is the CLI's own sentence ('Background command "…" completed (exit code 0)').

ADOPTED: `BackgroundTask.monitor` (the lane's launching tool was `Monitor`), `BackgroundTask.ambient` and `BackgroundTaskClose.monitor` (all serde-defaulted); an ambient or `skip_transcript` close journals no close entry; `killed` is terminal. The UI folds closes into "finished" rows titled by the CLI's sentence, names watches in the tray ("Watching “build log” · 1 command in the background"), and marks a turn that opens with no user message since the last turn end ("Woke on a monitor event · “build log”", "Resumed on its own") unless a finished row already shows the cause.

### Claude: other 2.1.281 frames (tolerated)

New system subtypes, all falling through the existing silent arm: `turn_preempted {reason:"rapid_followup", preempted_by_uuid, preempted_message_uuids}` (the CLI stopped a turn for a rapid follow-up — chimaera holds queued sends until the turn ends, so it cannot trigger it), `peer_message_hold`, `turn_handoff_available`, `dev_intent`, `per_turn_effort_changed`, `cloud_session_delta`, `upgrade_relay_marker`. Assistant frames carry `timestamp`, `request_id`, `wire_tool_inputs`, `tool_use_meta` (MCP display names/icons), `subagent_type`/`task_description` (subagent-attributed frames, still hidden).

### Codex 0.156.1

Two raw app-server probes against codex 0.156.1 (ChatGPT auth, `clientInfo.name: "chimaera"`, the driver's thread-open tuple), plus the generated 0.153.0 → 0.156.1 schema diff and a sweep of ~250 local rollouts. Pin bumped 0.153.0 → 0.156.1.

#### Reasoning summaries were never requested

Chimaera Codex sessions had received zero reasoning summaries (0 of ~25k reasoning items in the rollouts) while Codex Desktop, on the same models and account, got them on roughly a third of items. The cause is the model catalog: every model in the 0.156.1 bundled catalog (gpt-6-astra/sol/luna, gpt-5.6-sol/terra/luna, …) declares `"default_reasoning_summary": "none"`, so a thread whose `summary` setting was never set asks the model for no summary at all. `config/read` reports `model_reasoning_summary: null` when the user never configured one, and the thread's settings echo (`thread/settings/updated.threadSettings.summary`) is `null` for such a thread. Live: 0 of 3 turns without a summary setting streamed `item/reasoning/summaryTextDelta`; 3 of 3 turns with `turn/start.summary: "auto"` did (one `summaryPartAdded` + deltas, and the completed `reasoning` item's `summary` array filled, e.g. `["**Calculating the result**"]`). A single `"detailed"` turn produced none, so summary emission stays model-dependent even when requested. `turn/start.summary` ("for this turn and subsequent turns") is echoed at once as `thread/settings/updated {summary: "auto"}`. The rollout `turn_context.summary` field reads `"auto"` for both chimaera and Desktop turns and is therefore NOT the value sent to the model — don't use it as evidence.

ADOPTED: every `turn/start` carries `summary: "auto"` (Codex Desktop's value) unless `config/read` shows the user set `model_reasoning_summary` themselves, in which case nothing is sent and the config rules. The handshake now always issues the (2 s-bounded) `config/read`, not only when no effort was carried. Existing thought-stream mapping (`summaryPartAdded` → paragraph break, `summaryTextDelta` → `ThoughtChunk`) renders them unchanged. Live pin: `codex_turn_summary_setting_is_applied` asserts the settings echo and reports the delta count.

#### Subagent finish: the `completed` activity marker

`subAgentActivity.kind` is `started | interacted | interrupted | completed` in the 0.156.1 schema. `completed` arrives on the PARENT thread about 2 ms after the child thread's own `turn/completed` (live; all 133 local rollout pairs with usable timestamps agree — the other 42 are 0.144.2 child rollouts whose events share one timestamp), with item id `subagent-completed-<uuid>` rather than a collab call id. In 0.156.1 the `started` and `completed` markers arrive as an `item/started` + `item/completed` pair (the driver acts on `item/completed` only). The driver used to route `completed` into its catch-all "unseen kind" arm, which re-opened the already-closed stint as a new running `Agent: x` row (`agent:{thread}#N`) that nothing ever closed until teardown failed it. Now `completed` is a no-op for a closed row, closes a row only when the agent's own turn is not known to be running (its thread frames never reached the connection), and never opens one; unmined kinds open a row only for an unknown agent and otherwise just note the word.

Each stint's open→closed edge now also journals one `SubagentFinished {id: "agent:{thread}[#N]", label: <agentPath leaf>, status, result, stats}`. `status` is `completed` (turn completed), `stopped` (the `interrupted` marker, or a child turn ending `interrupted`), or `failed`. `result` is the stint's last child `agentMessage` whose `phase` is `final_answer` or null (commentary is skipped), trimmed and capped at `SUBAGENT_RESULT_MAX`. `stats` is the stint's tools, tokens and elapsed time in claude's spelling. Driver teardown fails open rows but journals no finish line, the same as claude. Live 0.156.1 child final message: `{type: "agentMessage", phase: "final_answer", text: "Forty-two"}`. No child `thread/started` reached the parent connection in the probe (the only `thread/started` was the parent's own; the child's first frame was `thread/status/changed {idle}`, before the spawn marker), so `agentNickname`/`agentRole` on the child thread object are not available from the stream — labels stay the `agentPath` leaf. `collabAgentToolCall tool:"wait"` still carries `receiverThreadIds: []` and `agentsStates: {}`, and spawn/followup still surface only as markers.

#### Failed turns are `turn/completed {status: "failed"}`

Neither the 0.153.0 nor the 0.156.1 `ServerNotification` union has `turn/failed`; `TurnStatus` is `completed | interrupted | failed | inProgress` and `Turn.error {message, codexErrorInfo, additionalDetails, misalignment}` is "only populated when the Turn's status is failed". The driver used to map any non-interrupted `turn/completed` to `TurnCompleted`, so a failed parent turn rendered as a success and a failed child turn closed its Agent row green. Now a failed parent turn is `TurnAborted {interrupted: false}` (reason = `turn.error.message`, or just "turn failed" when a terminal `error` notification already put the message in the transcript) and a failed child turn fails its row. The legacy `turn/failed` arms remain for older binaries.

#### `commandExecution.commandActions` → exploration rows

Live 0.156.1 shapes, present on `item/started` already: `{type: "read", command: "cat notes.txt", name: "notes.txt", path: "<absolute path>"}`, `{type: "listFiles", command: "ls", path: null}`, `{type: "search", command: "rg -n apple .", query: "apple", path: "."}` (path as typed), plus `{type: "unknown", command}`. `cwd` is a plain absolute path on the wire. ADOPTED: when every action is read/listFiles/search, the row is a `Read` row (only reads) or a `Search` row (any list or search) titled like the TUI's exploration lines — "Read a.rs, b.rs", "List src" / "List files", `Search "q" in src` / `Search "q"` (a `.` path is dropped), with segments joined by " · " — and `locations` are the read files (relative paths resolved against `cwd`). Any `unknown` action, or none, keeps the `Execute` row. Subagent progress lines use the same titles.

#### Collab tool titles

`CollabAgentTool` is `spawnAgent | sendInput | resumeAgent | wait | closeAgent | sendMessage | followupTask | interruptAgent | listAgents`. None except `wait` was observed as an item, but each now gets a readable title that names its `receiverThreadIds` by the tracked agents' names (an untracked id shows as `agent …<last 6>`; at most three names and then "+N more"): "Message → agent_a: …", "Follow-up → agent_a", "Interrupt agent_a", "List agents", "Spawn agent: …", "Input → …", "Resume …", "Close …". Unknown tools keep "collab {tool}".

#### Other 0.156.1 facts

- `thread/tokenUsage/updated {threadId, turnId, tokenUsage: {total, last, modelContextWindow}}` breakdowns carry `inputTokens, cachedInputTokens, cacheWriteInputTokens, outputTokens, reasoningOutputTokens, totalTokens`, with `totalTokens = inputTokens + outputTokens`, so `outputTokens` already includes reasoning. ADOPTED as `TurnTokens` for the parent thread only. The value is `total.outputTokens` minus a base taken at the turn's first update (`total − last`, so a resumed thread's history never counts). An update carrying another turn's id is ignored. A new value is journaled only after a move of at least 256 tokens, and once more at turn end if one was held back.
- `thread/start`/`thread/resume` have no `effort` param in the schema: the `effort` the driver puts on the open request is silently ignored (probe: `"high"` requested, settings echo `"xhigh"` — the config default). The existing post-handshake `SetEffort` reconcile already covers this.
- `agentMessage` gained `delivery: "async" | null` and `questions: AsyncUserInputQuestion[] | null` beside `phase: "commentary" | "final_answer" | null` (the schema says providers are inconsistent, so treat null as unknown). Commentary continues to render as prose; no change.
- `ThreadSettingsUpdateParams.multiAgentMode` is now `@deprecated Ignored. Use effort: "ultra" for proactive multi-agent behavior`.
- `model/list` (Pro, 0.156.1): `gpt-6-astra` (default), `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5` (legacy, retiring 2026-10-14), plus hidden `gpt-reserve`, `codex-auto-review`. The launcher fallback lists the first three plus `gpt-5.6-sol`.

#### Gate

Hermetic mapper tests (9 new in codex.rs): the completed marker never re-opens a closed row, and closes only a row without a running turn; one finish per stint with status words and stint stats, a capped answer, and no finish on teardown; exploration rows (read, multi-read, list, search, a mixed `unknown` keeps Execute, relative-path resolution, subagent progress label); collab titles; `turn/start.summary` present by default and absent when configured; failed-status turn end aborts and does not repeat the message; `TurnTokens` base, throttle, child isolation, and settle-at-end. Live: `codex_turn_summary_setting_is_applied` (new) and extra assertions in `codex_collab_subagent_surface` (the completed marker trails the child turn end; the child's last message is not commentary). These run under `just chat-smoke` and need codex 0.156.1 on PATH (the global install here is still 0.153.0).
...
## Pass 32 (2026-09-25 — binary mining claude 2.1.259, no live probe): what a daemon stop does to claude's background work. ADOPTED.

The trigger: a daemon update resumes a chat's conversation, but the background commands, monitors and workflows its old process was running never come back, and nothing tells the agent. Sources: the 2.1.259 binary's embedded JS and its SDK schema `.describe()` text. No paid probe; the local CLI was logged out.

- **Claude's Bash tool starts its shells detached.** The shell provider is `{type: "bash", shellPath, detached: true, stdin: "pipe", …}`, so every command, a `run_in_background` one included, runs in its own session and process group. Claude stops a task by signalling that group (`process.kill(-pid, …)`, SIGTERM and then SIGKILL after 5 s). A SIGKILL to the claude process runs none of that, so its background shells outlive it and are reparented to init. Until this pass, a daemon stop killed chat drivers only through the tokio runtime's drop-time SIGKILL (`kill_on_drop`), which left a background dev server holding its port and a Monitor's `tail -f` running forever on the host.
- **Closed stdin makes claude kill held background tasks itself.** The initialize schema's `perTaskStopAffordance` text: "Closed-input exception: a one-shot run (string prompt / -p closes stdin) still kills hold-back tasks at the held-result release regardless of the declaration — with stdin closed, a stop_task control could never be delivered, so the fail-closed kill stands." The same text says that without the declaration an interrupt also kills background agents and workflows.
- **Neither agent restarts that work on resume.** `--resume` and `thread/resume` restore the conversation; its processes are gone. The transcript still holds the agent's own tool results (for example "Command running in background with ID: bg-1"), so the agent can restart the work once it is told.

ADOPTED (server side; the only driver change is below): a graceful daemon stop ends every live chat driver through the harness's polite path (stdin closed, `KILL_GRACE`, then SIGKILL) before exiting, after the ledger's final flush (`chat::stop_all_for_exit`). The pump folds a per-session `Carryover` (bridge on, ultracode, turn in flight, running non-ambient background tasks), which the ledger snapshots. Resurrection restores the bridge and ultracode through the spawn. When work was cut off, it sends the agent one message listing the tasks with their kinds, labels and ids; the echo carries `UserMessage.origin: "restart"`, stamped by the manager's Send↔echo reservation, so neither driver carries the tag. Driver change: `SpawnSpec.initial_ultracode` makes the claude handshake apply `apply_flag_settings {ultracode: true}` under `bootstrapping`, after effort and mode (ultracode raises effort itself), so the read-back is not recorded as a pick.

Unverified live: that a stream-json claude that is idle but holds background tasks exits within the 3 s grace on stdin EOF and kills them first. A mid-turn claude may not exit within 3 s; it is then SIGKILLed as before, and its detached shells survive. Probe (a few cents with Haiku): start a `run_in_background` `sleep 600` and a Monitor, close stdin, then check `pgrep sleep` and the exit time.

### Gate (Pass 32)

Hermetic: the `Carryover` fold (bridge through Init snapshots and RemoteControl, ultracode, turn start and end, running and non-ambient filtering, reset on exit), origin stamping pairing only with a Send echo, `carryover_reports_process_state_and_ultracode_restores` (fake-claude: bridge, bootstrapped ultracode read back unchosen, tagged echo, background set, reset on exit), the ledger round trip with carryover (older or malformed entries load as `None`), and the message builder. Live on an isolated daemon with fake-claude, four daemon lives: bridge and ultracode restored, the message listing `bg-0` sent once and the agent restarting the task, a bridge the user had turned off staying off while `chat.remoteControlAtStart` was on, a cut-off turn told to continue, no message with `chat.resumeAfterRestart` off, clean driver exits (`exited status 0`) and a 0.19 s daemon stop with two chats. `just chat-smoke` was not run: the local claude was logged out.
