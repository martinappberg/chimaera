...
- **Codex TUI takes the chimaera MCP server by `-c` too** (0.153.0, live): a TUI spawned with `-c mcp_servers.chimaera.url="…"`, `-c mcp_servers.chimaera.bearer_token_env_var="CHIMAERA_MCP_KEY"` (key in the PTY env, never argv) and per-tool `-c mcp_servers.chimaera.tools.<t>.approval_mode="approve"` starts cleanly and its `/mcp` lists `chimaera: connected (5 tools)` (3 linked-terminal + 2 notes). ADOPTED for codex TUIs **only while a plugin with tools is active in the workspace** (`launcher::codex_tui_mcp_args`); with none on, the argv is unchanged. Unconfirmed: that `approval_mode="approve"` suppresses the TUI's per-call prompt — the probe account's login had lapsed before a model turn could run.
