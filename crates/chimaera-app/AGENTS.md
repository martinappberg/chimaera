...
| `shell/connect.rs` | The `connect` flight state machine (one coalesced ssh attempt per host) + the host-row wire vocabulary (`HostState`/`HostStatus`) + `with_hosts`, the only way hosts.json is touched (off the reactor via `spawn_blocking`). |
