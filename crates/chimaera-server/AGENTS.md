...
| `drafts.rs` | The draft mirror (`/fs/drafts`, `/fs/draft`): unsaved editor text, so a window on another origin (a new tunnel port) can recover it. One 0600 JSON file per draft under `<data>/drafts/` (0700), named by SHA-256(path); ≤1 MiB text, ≤64 drafts, ≤16 MiB total, least-recently-updated evicted by file mtime; corrupt files skipped. |
