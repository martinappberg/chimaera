...
| `plugins/` | Workbench plugins and the **plugin host** (plan: `docs/plugin-system-plan.md`). `mod.rs`: the catalog — WASM plugins embedded from `plugins/dist` (rust-embed, like the UI; built by `scripts/build-plugins.sh` from the separate `plugins/` cargo workspace — **a missing `plugins/dist` fails the build**, as a missing `web-ui/dist` does) sorted by id (the Plugins page's order, and the order active plugins' tools and paragraphs reach agents in); every plugin's behaviour is its component (`Manifest.wasm`), none is daemon code; test builds also embed `plugins/dist-test` and `test_catalog` adds the fixture; per-workspace switch (`Workspace.plugins_on`, a flip resets the plugin's instance and fault), footprint detection off the reactor, install and setup routes, an additive `fault` on the card when a plugin can't answer. `runtime.rs`: one wasmtime engine per process (Cranelift, epoch interruption, 64 MiB reservation per memory), one 100 ms ticker thread, each component compiled once off the reactor, ≤64 instances per (plugin, workspace) behind an async mutex (LRU, 10 min idle sweep on access), per-call deadlines (5 s; 30 s knowledge), a trap drops the instance and 5 in a minute fault the plugin there; typed entry points (`offer` checks the component's tools against the manifest, `call_tool`, `knowledge` — `knowledge.rs`'s provider call, 30 s — `on_event`, `hook` (every active plugin, so a provider hears hooks too), `session_ended`) and the `emit` ring `/ws/events` sends as `plugin` frames. `hostfns.rs`: the WIT `host` interface, every limit in one place — workspace-relative `O_NOFOLLOW` fs under `fs::FILESYSTEM_WORK` (8 MiB / 4,096 caps), 64 KiB state per (plugin, workspace) (`PluginStates`), `note`-only Timeline appends on `notes::take_post_slot`, sessions, emit, log; serves the host's own call context, never the guest's `cx`. `tools.rs`: generic — `owner` from the manifests, `offered`/`call` through the runtime (MCP tools offered AND call-gated only where active, pre-allowed at spawn). Recipe: `docs/agent-guides/plugins.md` (the plan's docs phase rewrites it for WASM). |
