...
| `compute.rs` | Slurm awareness: login-shell detection (cached; `CHIMAERA_SLURM_BINDIR` test knob) + `GET /compute` — the user's queue + partitions via capped/timeout-fenced `squeue`/`sinfo`, 30s single-flight snapshot cache. Never a 500: a failed `squeue` CALL carries the previous jobs forward tagged `degraded` (distinct from an empty queue); everything else degrades to an empty snapshot. |
