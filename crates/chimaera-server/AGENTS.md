...
| `board.rs` | Board routes (`/board/render` → content-addressed PNG + diagnostics sidecar + `/raw` ticket, `/board/describe`, `/board/edit` — one semantic gesture (move/resize/replace-text/`set`, the board-level `canvasBackground` op — no object, journaled `canvas-changed`, or the multi-object `arrange` op — `{op, objects}` for align/distribute/`snap-grid`, journaled as one `move` per object, plus the two structural verbs `group`/`ungroup`, which answer with the minted/dissolved `group` id + `members` and journal `object-added`/`object-removed` (their refusals are 422s, raised before any write); a `move` on a `group` translates its page-absolute children so it moves as a unit, taking its delta from the child **union** (`normalize::effective_frame`) because a group's stored `at` is derived and a hand-authored group has none), saved through the crate's canonical writer, then journaled best-effort as actor `human` with the seq echoed as `journalSeq`; render takes an optional `mode: "light"|"dark"` that an `auto`/absent board theme resolves to (absent = dark; a pinned theme ignores it) — but a **literal `#rrggbb` `canvas.background` outranks the mode** for the tiers that were following it, because a literal ground is painted verbatim and light-mode ink on a black canvas is unreadable; both render and export funnel through `board::resolve_theme` → `theme::resolve_for_board` for that, and the response's `schemes[].variant` names the *effective* variant (what picking that scheme would give), never the requested mode's. The cache keys on the RESOLVED theme, so a literal-ground board's two modes correctly share one entry while a token-ground board's stay distinct; `/board/journal` GET (events after a seq, ≤500 + `latestSeq`) / POST (one validated event, seq assigned server-side); `/board/export` → pptx/pdf/svg into `.chimaera/board/exports/` behind a `/download` ticket, a multi-page SVG export ticketing a directory). Mutating routes serialize per canonical path on a bounded 16-shard lock; a mutation bumps the per-workspace **board epoch** (`{"type":"board","epochs":…}` on `/ws/events`, invalidate-and-pull) immediately while the git bump defers behind a ~1s per-path settle so a layout session costs one `git status`. Thin wrappers over `chimaera-board`; blocking work rides `fs.rs`'s semaphore. |
