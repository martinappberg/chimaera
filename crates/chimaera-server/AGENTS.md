...
| `git/` | Read-only git, split into `resolve`/`parse`/`service`/`worktree`/`http`: porcelain-v2 status (single-flighted per workspace with a ~1s shared result, so an epoch bump's fan-out of window refetches runs one `git status`, not one per window), side-by-side diff, worktree orchestration (confined to a managed root), login-shell git resolution gated at >=2.15. |
