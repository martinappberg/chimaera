...
| `ws.rs` | WebSockets: `/ws/sessions/{id}` (PTY byte pipe; 1 MiB frames chunked to the input queue; output coalesced — leading-edge first chunk, then one frame per ~8 ms tick or 32 KiB; attach/resync snapshot renders run under `spawn_blocking`, off the reactor), **`/ws/chat/{id}`** (structured events; 10 MiB command frames and ~512 KiB replay batches), `/ws/events` (the session-list bus). |
