...
| `board.rs` | Board routes (`/board/render` → content-addressed PNG + diagnostics sidecar + `/raw` ticket, `/board/describe`, `/board/edit` — one semantic gesture, saved through the crate's canonical writer). Thin wrappers over `chimaera-board`; blocking work rides `fs.rs`'s semaphore. |
