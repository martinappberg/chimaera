...
| `upload.rs` | `POST /sessions/{id}/upload?name=` — the landing pad for OS-desktop drops + pasted screenshots. STREAMS the body to `uploads_root/<session-id>/` (hidden-tmp-then-rename), capped 64 MB/file + 256 MB/session + 256 files, strict basename sanitize (no traversal), bearer-authed; the per-session dir is pruned on session delete/retire/shutdown/boot-sweep. Also `save_send_images`, called by `ws.rs` on every chat `Send`: each image block gets a saved copy here (same caps, decode + writes on the blocking pool) and its `path` stamped — a client-supplied `path` is always cleared first, and a failed save leaves `None` without refusing the send — so the drivers' `UserMessage` echo can carry `attachment_paths`. |
