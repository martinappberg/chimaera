...
| `update.rs` | The self-update reporter (`GET /update`, `?refresh=true` = check now; test knobs `CHIMAERA_RELEASES_API`/`UPDATE_CURRENT`). A failed check is a `state: "failed"` with the reason, never "no update"; checks serialize on `AppState.update_check`. |
