...
//! prepended to a chimaera session's PATH via spawn env only. The dir also
...
pub(crate) fn write_shims(
    ...
    // The `chimaera` shim is unconditional (unlike the agent shims): the
    // daemon binary serving this session always exists, and resolving
    // `current_exe()` at write time keeps the shim honest across restarts
    // from a new binary — on remote hosts it is the deployed binary. A
    // resolution failure skips only this shim, never the agent shims.
    match std::env::current_exe() {
        Ok(daemon_bin) => write_atomic(
            &shim_dir.join("chimaera"),
            chimaera_shim_script(&daemon_bin).as_bytes(),
            0o755,
        )?,
        Err(err) => {
            tracing::warn!(%err, "cannot resolve the daemon binary; skipping the chimaera shim");
        }
    }
    Ok(())
}
...
# The daemon's own binary, so `chimaera board show` (and the rest of the CLI)
# works inside any chimaera session without a user install.
exec {bin} "$@"
"#,
        bin = sq(&daemon_bin.display().to_string()),
    )
}
...
mod tests {
    ...
    /// The `chimaera` shim is unconditional — even with nothing managed and
    /// nothing explicit, every session PATH carries the daemon's own binary
    /// as `chimaera`, so `chimaera board show` resolves in any workspace
    /// without a user install. In tests `current_exe` is the test binary;
    /// never exec the shim here (it would re-run the whole test suite).
    #[test]
    fn chimaera_shim_execs_the_daemon_binary() {
        use std::os::unix::fs::PermissionsExt;

        let dir = test_dir("chimaera-shim");
        let shim_dir = dir.join("shims");
        let managed = dir.join("agents").join("bin");
        std::fs::create_dir_all(&managed).unwrap();
        write_shims(&shim_dir, &managed, &HashMap::new()).unwrap();

        let path = shim_dir.join("chimaera");
        let script = std::fs::read_to_string(&path).unwrap();
        assert!(script.starts_with("#!/bin/sh"), "{script}");
        let me = std::env::current_exe().unwrap();
        assert!(
            script.contains(&format!("exec {}", sq(&me.display().to_string()))),
            "{script}"
        );
        ...
        // A quote in the daemon path survives the shim's own quoting (sq —
        // the module-wide interpolation rule).
        let quoted = chimaera_shim_script(Path::new("/opt/o'brien/chimaera"));
        assert!(
            quoted.contains(r#"exec '/opt/o'\''brien/chimaera' "$@""#),
            "{quoted}"
        );
