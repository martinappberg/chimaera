...
| `src/layout.rs` | text measurement/wrapping over usvg's own `fontdb` + rustybuzz; the `bundled` module bakes the brand text fonts into the binary (`include_bytes!` + `load_font_data` under `fonts/text/`: **Geist** the default sans, **IBM Plex Sans** a selectable neutral alternate, **JetBrains Mono** for `code`), registered between vendored and system so a fontless HPC node renders on-brand and deterministically — the fix for the fallback gap. Static weights only (exact weight selection); provenance/licenses (all SIL OFL 1.1) in `fonts/text/README.md` |
