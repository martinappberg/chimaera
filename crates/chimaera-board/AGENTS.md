...
| `src/diagram.rs` | the `diagram` composite: deterministic layered layout (Sugiyama-lite + neighbor-mean refinement, in-crate — no maintained dagre exists; per-node `at` pins win verbatim) with rounded orthogonal edge routing (per-edge ports, channel tracks, side lanes for loops/skips — edges never overlap or cross nodes), chip-backed edge labels, themed node paint, `natural_size` for `show --mermaid` auto-fit; expands to primitives at render; the mermaid flowchart import |
