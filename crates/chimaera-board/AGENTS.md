...
| `src/lint.rs` | the legality, target and style profiles + `lint_fix`; findings always name object, field, and the numbers. `--style` measures near-miss alignment (to a peer, and to the `canvas.grid` lines) + a gentle off-layout-grid Info nudge; `lint_fix` snaps near-misses to the peer or the 8 pt-quantized grid |
| `src/arrange.rs` | align/distribute/grid over named objects, one pure function; refuses slot-placed targets (their geometry is derived). `arrange_ids` is the server-facing verb+id-set wrapper (adds `snap-grid`: snap a selection's `at` to `canvas.grid`); every verb moves via `schema::translate_object` so aligning a group carries its children |
